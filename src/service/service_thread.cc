...
static DWORD create_process(std::wstring const &cmd, HANDLE &hProcess, HANDLE &hThread)
{
...
static DWORD open_process(DWORD process_id, HANDLE &hProcess)
{
	DWORD pid = 0;
...
static DWORD query_process_id(std::wstring const &name = {})
{
...
HANDLE start_process()
{
	...
	auto pid = query_process_id(std::format(L"{}.exe"sv, MT_NAME));
	if (pid)
	{
	...
	pid = create_process(_expath, handle, hthread);
	if (pid)
	{
		CloseHandle(hthread);
	}
